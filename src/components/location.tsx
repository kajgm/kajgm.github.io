export function Location(props: { location: string; locLink: string }) {
  ...
  return (
    <a href={locLink} className="flex flex-wrap group">
      <svg
        className="mt-1 w-4 h-4 fill-light-sub-text-color group-hover:fill-red-800 dark:fill-dark-sub-text-color dark:group-hover:fill-red-800 group-hover:transition-color group-hover:duration-300 group-hover:ease-in-out"
      ...
      <p className="m-0 text-md font-medium pl-1 text-light-sub-text-color dark:text-dark-sub-text-color">
        {location}
      </p>
    </a>
  );
